...
// ---------------------------------------------------------------------------
// Aggregation type (from pivot-sql)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// aggToSQL — string-based aggregation → SQL (used by PivotIndicator maps)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// toSQL — unified AST → SQL (handles ref, field, literal, agg, call)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// getIndicatorSQLMap — build indicator → SQL map from PivotConfig
// ---------------------------------------------------------------------------

export interface PivotIndicatorForMap {
  key: string
  field: string
  aggregation: AggregationType
}

export interface CalculatedIndicatorForSort {
  key: string
  logic: ExpressionNode
}

export function getIndicatorSQLMap(
  indicators: PivotIndicatorForMap[]
...
// ---------------------------------------------------------------------------
// topoSortCalculated — topological sort of calculated indicators
// ---------------------------------------------------------------------------

export function topoSortCalculated(
  calculated: CalculatedIndicatorForSort[]
): CalculatedIndicatorForSort[] {
  const byKey = new Map(calculated.map((c) => [c.key, c]))
  const visited = new Set<string>()
  const visiting = new Set<string>()
  const result: CalculatedIndicatorForSort[] = []
